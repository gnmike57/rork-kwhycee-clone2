...
struct ProfileCreationView: View {
        ...
        VStack(spacing: 32) {
            ...
            VStack(spacing: 8) {
                ...
                if scanner.isScanning {
                    Text("Scanning devices, microphones, web fingerprint,\nand running Loom media comparison test…")
                        .font(.subheadline)
        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 6) {
                    ...
                    Text("All device specs, microphone & media descriptors captured")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                }
    ...
    private func resultsSummary(_ profile: DeviceProfile) -> some View {
        VStack(spacing: 0) {
            ...
            if let test = profile.mediaTestResult {
                ...
                summaryRow(icon: "video.slash", label: "Loom Media", value: "Not tested")
            }
        }
        .background(Color(.secondarySystemGroupedBackground))
    ...
    private func cameraCard(_ cam: CameraDeviceSpec) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [
                ...
                if let aperture = cam.lensAperture {
                    specChip("Aperture", String(format: "f/%.1f", aperture))
                }
    ...
    private func saveButton(_ profile: DeviceProfile) -> some View {
        Button {
            ...
            Label("Save & Use Profile", systemImage: "checkmark.circle.fill")
                .font(.headline)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 14)
                .background(.cyan, in: .rect(cornerRadius: 14))
