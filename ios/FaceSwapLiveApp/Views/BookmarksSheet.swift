            ...
            Group {
                if viewModel.bookmarks.isEmpty {
                    ...
                    List {
                        ...
                        .onDelete { offsets in
                            // Deleting one item shifts every index after it, so the
                            // bookmarks must be resolved before any of them is removed.
                            let doomed = offsets.map { viewModel.bookmarks[$0] }
                            doomed.forEach(viewModel.removeBookmark)
                        }
                    }
                }
            }
