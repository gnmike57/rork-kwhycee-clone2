...
nonisolated struct ImageProcessingResult: Sendable {
    let overlay: UIImage
    let aspectRatio: CGFloat
}

nonisolated struct CaptureContext: Sendable {
...
nonisolated extension CGImagePropertyOrientation {
    /// Whether applying this orientation turns a landscape buffer portrait.
    var swapsAxes: Bool {
        switch self {
...
/// Vision and Core Image work for the Preview tab.
///
/// Holds no mutable state: the one `CIContext` is thread-safe, so the same
/// processor serves the capture queue and the source-photo worker alike.
nonisolated final class ImageProcessor: Sendable {
    ...
    /// Finds the largest face in a camera frame.
    ///
    /// `orientation` is the same one the frame is displayed with, so the
    /// result is reported in the upright picture's coordinates.
    func detectFeatures(in pixelBuffer: CVPixelBuffer, orientation: CGImagePropertyOrientation) -> ImageAnalysisResult? {
        ...
        let effectiveWidth = orientation.swapsAxes ? rawHeight : rawWidth
        let effectiveHeight = orientation.swapsAxes ? rawWidth : rawHeight
    ...
    /// Bakes the overlay into one frame exactly where the screen showed it.
    func compositeCapture(
        pixelBuffer: CVPixelBuffer,
        orientation: CGImagePropertyOrientation,
        context: CaptureContext
    ) -> UIImage? {
        let orientedCI = CIImage(cvPixelBuffer: pixelBuffer).oriented(orientation)
